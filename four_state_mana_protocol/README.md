...
Run `Mana_protocol_4states_log2_embedded_0714.m` in Matlab. The script creates a `figures` directory under Matlab's current working directory and writes the basename `Mana_log2_protocol_d3_N5_theta_0p2_Nr1000` in PNG, vector PDF, EPS, FIG, and MAT formats. Existing files with those names may be overwritten. The committed `Mana_0.2_4states.png` and `Mana_0.2_4states.pdf` are preview files and are not the filenames generated by the current script.
