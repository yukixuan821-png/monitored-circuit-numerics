...
For a fixed `(d, N)`, all values of `theta_M` use the same `Nr` and `T_max`, so the angle dependence is compared under the same numerical budget. The burn-in length is `0`. Each data point is obtained by averaging the final-time mana over `Nr` independent trajectories, and the error bars are standard errors `std / sqrt(Nr)`. The small-angle power-law slope is fitted over the range `theta_M < 0.1`.
